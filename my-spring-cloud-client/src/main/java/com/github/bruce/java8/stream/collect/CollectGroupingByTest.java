...
public class CollectGroupingByTest {
    ...
    public static void main(String[] args) {
        ...
        line(ownerWeightMap);
        // 获取每个owner的最轻的苹果
        Map<String, Optional<Apple>> ownerMaxWeightMap = apples.stream().collect(groupingBy(Apple::getOwner, minBy(comparingInt(Apple::getWeight))));
        line(ownerMaxWeightMap);
        // 或者直接get出来，不是Optional
        Map<String, Apple> ownerMaxWeightMap1 = apples.stream().collect(toMap(Apple::getOwner, Function.identity(), BinaryOperator.minBy(comparing(Apple::getWeight))));
